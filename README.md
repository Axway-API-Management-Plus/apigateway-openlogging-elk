...
However, without further customization, the base setup uses minimal parameters to test the solution on a machine with at least 16 GB of RAM including the API-Management platform _(For instance like the Axway internal API-Management reference environment.)_. Therefore, the solution is __not suitable for a production__ environment __without further configuration__. For a production environment, check the parameters mentioned in the `env-sample` at the beginning and set them if necessary. 
