...
const { mergeCustomPropertiesIntoIndexTemplate, mergeCustomPropertiesIntoTransform } = require('./customProperties');
...
async function getPlugin(pluginConfig, options) {
	const cache = new NodeCache({ stdTTL: pluginConfig.lookupCacheTTL, checkperiod: 120, useClones: false });
	...
	sdk.load(path.resolve(__dirname, 'flow-nodes.yml'), {lookupCurrentUser, lookupTopology, lookupAPIDetails, getCustomPropertiesConfig, mergeCustomPropertiesIntoIndexTemplate, mergeCustomPropertiesIntoTransform, getAPIManagerConfig, getAPIManagerOrganizations, isIgnoreAPI, lookupApplication }, { pluginContext: { cache: cache }, pluginConfig});
