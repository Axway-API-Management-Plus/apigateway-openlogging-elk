...
describe('Test Application-Lookup endpoint', function () {
	...
	describe('trace endpoint tests', () => {
		...
		it('[applookup-0001] Should return http 200 with Application details for an existing applications', () => {
			...
			return requestAsync({
				method: 'GET',
				uri: `http://localhost:${server.apibuilder.port}/api/elk/v1/api/lookup/application/268fd3bd-3af6-4565-a3aa-6dc9559f1fbf`,
				auth: auth,
				json: true
			}).then(({ response, body }) => {
		...
		it.only('[applookup-0002] Should return http 200 for an unknown application - ID is the name', () => {
			...
			return requestAsync({
				method: 'GET',
				uri: `http://localhost:${server.apibuilder.port}/api/elk/v1/api/lookup/application/268fd3bd-3af6-4565-a3aa-UNKNOWN-APP`,
				auth: auth,
				json: true
			}).then(({ response, body }) => {
				expect(response.statusCode).to.equal(200);
